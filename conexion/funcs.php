<?php
	...
	function usuarioExiste($usuario)
	{
		...
		$stmt = $mysqli->prepare("SELECT idclientes FROM tbl_clientes WHERE usuario = ? LIMIT 1");
	...
	function emailExiste($email)
	{
		...
		$stmt = $mysqli->prepare("SELECT idclientes FROM tbl_clientes WHERE correo = ? LIMIT 1");
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->Host = 'smtp.gamil.com'; //Modificar
		$mail->Port = 25; //Modificar
		
		$mail->Username = 'ssoltcar@gmail.com'; //Modificar
		$mail->Password = 'ssoltcar2019'; //Modificar
		
		$mail->setFrom('ssoltcar@gmail.com', 'Password'); //Modificar
	...
	function validaIdToken($id, $token){
		...
		$stmt = $mysqli->prepare("SELECT activacion FROM tbl_clientes WHERE idclientes = ? AND token = ? LIMIT 1");
	...
	function generaTokenPass($user_id)
	{
		...
		$stmt = $mysqli->prepare("UPDATE tbl_clientes SET token_password=?, password_request=1 WHERE idclientes = ?");
	...
	function verificaTokenPass($user_id, $token){
		...
		$stmt = $mysqli->prepare("SELECT activacion FROM tbl_clientes WHERE idclientes = ? AND token_password = ? AND password_request = 1 LIMIT 1");
